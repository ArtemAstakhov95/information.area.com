...
class Tables{
    ...
    public function addField($fieldName, $fieldType, $fieldTitle, $autoincrement, $displayedit, $displayBrowse){
        $this->fields[$fieldName] = new Field($fieldName, $fieldType, $fieldTitle, $autoincrement, $displayedit, $displayBrowse);
    }
    ...
    public function displayBrowse(){
        ...
        $this->result = '<div class="cont admin-browse-view" style="width: 960px; margin-left: auto; margin-right: auto;">';
        if($this->searchField){
        ...
        $this->result .='<h2>'.$this->browseTitle.'</h2><span id="add-news-icon" title="Додати сторінку"><a href="/'.$this->table_name.'/add"><i class="fa fa-plus fa-2x"></i> </a></span>';
        if(self::$searchValue){
            //$this->result .= '<h3>Поточне поняття</h3>';
        }
        $this->result .= '<div class="admin-view-tabel-div"><table class="admin-view-table"><tr><th></th>';
        if($this->subTable){
            $this->result .= '<th></th>';
        }
...
class Field{
    public $name, $type, $title, $autoincrement=false, $element, $displayEdit, $displayBrowse;
    public $isLookup=false, $lookupTable, $lookupFieldName, $lookupCaptionName, $lookupValue = array();

    public function __construct($name, $type, $title, $autoincrement, $displayEdit, $displayBrowse){
        $this->name = $name;
        $this->type = $type;
        $this->title = $title;
